...
#end if
	Private mFirstVisibleIndex = -1, mLastVisibleIndex = 0x7FFFFFFF As Int
...
Private Sub InsertAtImpl(Index As Int, Pnl As B4XView, ItemSize As Int, Value As Object, InitialSize As Int)
	...
	If Index < mLastVisibleIndex Or GetItem(mLastVisibleIndex).Offset + _
			GetItem(mLastVisibleIndex).Size + DividerSize < GetScrollViewVisibleSize Then
		UpdateVisibleRange
	End If
End Sub
